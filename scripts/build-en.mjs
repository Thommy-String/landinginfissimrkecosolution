#!/usr/bin/env node
/**
 * Build EN version of the landing page.
 *
 * Strategy:
 ...
 *  3. Override vite base to '/windowfactory/' and run `vite build` from that temp folder
 *  4. Move the resulting build output into ./dist-en at the repo root
 *
...
const TMP = join(os.tmpdir(), `mrk-en-build-${Date.now()}`)
const FINAL_OUT = join(ROOT, 'dist-en')

console.log('🌍  Building English version → /windowfactory/')
...
// ─────────────────────────────────────────────────────────────
const SKIP = new Set(['node_modules', 'dist', 'dist-en', '.git', '.vscode', 'scripts'])
...
const DICTIONARY = [
  ...
  ['it_IT', 'en_GB'],
  ['lang="it"', 'lang="en"'],
  ['ecosolutionsas.com/fabbricainfissi/', 'ecosolutionsas.com/windowfactory/'],

  // ── PHONE NUMBER (IT → UK) ───────────────────────────────
  ['+39 334 222 1212', '+44 7825 186822'],
  ['tel:+393342221212', 'tel:+447825186822'],
  ['wa.me/393342221212', 'wa.me/447825186822'],
...
// ─────────────────────────────────────────────────────────────
// 4) PATCH vite.config.js → base '/windowfactory/'
// ─────────────────────────────────────────────────────────────
const viteCfg = join(TMP, 'vite.config.js')
let cfg = readFileSync(viteCfg, 'utf8')
cfg = cfg.replace(/base:\s*['"][^'"]+['"]/, `base: '/windowfactory/'`)
writeFileSync(viteCfg, cfg, 'utf8')
console.log(`✅  Patched vite.config.js → base '/windowfactory/'`)
...
if (isDev) {
  console.log('🚀  Starting EN dev server (Ctrl+C to stop)…')
  console.log('   → open http://localhost:5173/windowfactory/')
  // Forward stdio so Vite output is visible & Ctrl+C works.
  // --open /windowfactory/  makes the browser jump straight to the right path
  // (otherwise Vite warns: "server configured with public base URL of /windowfactory/")
  execSync('npx vite --host --open /windowfactory/', { cwd: TMP, stdio: 'inherit' })
  // Cleanup on exit
...
console.log('\n✨  Done! Upload the contents of  dist-en/  to')
console.log('    https://www.ecosolutionsas.com/windowfactory/')
